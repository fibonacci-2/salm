#!/bin/bash
#SBATCH --job-name=gpt-m-l40s
#SBATCH --output=gpt-m-l40s%j.out
#SBATCH --error=gpt-m-l40s%j.err
...
OUTPUT_DIR="checkpoints/gpt-m-l40s"
mkdir -p "$OUTPUT_DIR"
...
python -m torch.distributed.run --standalone --nproc_per_node=2 models/salm.py \
