#!/bin/bash
#SBATCH --job-name=gpt-s-v100
#SBATCH --output=gpt-s-v100%j.out
#SBATCH --error=gpt-s-v100%j.err
...
OUTPUT_DIR="checkpoints/gpt-s-v100"
mkdir -p "$OUTPUT_DIR"
...
torchrun --standalone --nproc_per_node=1 models/salm.py \
    --output_dir "$OUTPUT_DIR" \
