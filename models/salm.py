...
from tokenizers import Tokenizer                                    # ← changed: local tokenizer
...
total_batch_size = 131072   # tokens per gradient step (B * T * grad_accum_steps)
B = 8                       # micro batch size
T = 512                     # sequence length

# Corpus: 248,407,644 train tokens (277M total, ~10% held as val)
# 1 epoch = 248,407,644 / 131,072 = ~1,895 steps
...
# ── Tokenizer ────────────────────────────────────────────────────
# changed: load local 32k BPE tokenizer instead of Aranizer-PBE-64k
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        config_args['vocab_size'] = 32000                              # ← changed: 64000 → 32000
...
class DataLoaderLite:
    def __init__(self, B, T, process_rank, num_processes, split):
        ...
        print(f'reading shards from {data_root}')
...
model = GPT(GPTConfig(vocab_size=32000))                           # ← changed: 64000 → 32000
model.to(device)
use_compile = False
if use_compile:
...
max_lr = 6e-4
min_lr = max_lr * 0.1
warmup_steps = 315          # ~3% of max_steps (scaled from corpus size)
def get_lr(it):
    if it < warmup_steps:
...
for step in range(max_steps):
    ...
    if ((step > 0 and step % 500 == 0) or last_step) and (not use_compile):
        ...
        max_length = 256
        # ── changed: .encode() returns an Encoding object; use .ids for the token list ──
        tokens = enc.encode("السلام عليكم").ids
        ...
        for i in range(num_return_sequences):
            ...
            # ── changed: enc.decode() takes a list[int] directly (no skip_special_tokens arg) ──
